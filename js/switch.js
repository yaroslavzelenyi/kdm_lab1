...
window.addEventListener('DOMContentLoaded', () => {
        ...
        if (target.classList.contains('switcher__item')) {
            ...
            if (!target.classList.contains('forward')) {
                switchBlock.style.cssText +=
                    'transform: translateX(120px);transition: transform 0.3s cubic-bezier(0.645, 0.045, 0.355, 1)';
            } else {
                switchBlock.style.cssText +=
                    'transform: translateX(0);transition: transform 0.3s cubic-bezier(0.645, 0.045, 0.355, 1)';
            }
        }
    });
    ...
    window.addEventListener('click', (event) => {
        let target = event.target;
        if (!target.classList.contains('table__input')) {
            input.forEach((item) => {
                if (item.value == '') {
                    item.value = '0';
                }
    ...
    // tableA.addEventListener('click', (event) => {
    //     let target = event.target;
    //     if ((target.classList.contains = 'table__input')) {
    //         input.forEach((el) => {
    //             el.value = '';
